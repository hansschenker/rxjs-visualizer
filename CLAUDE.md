...
- `STEP_MS` sets four things at once: source cadence, reveal cadence, arrow transition time (passed to the renderer as `stepMs`, written to the `--rx-step` CSS variable), and the width of one shaded time band. Keep them coupled. `PX_PER_STEP / STEP_MS` is the X scale, so changing speed does not change layout.
