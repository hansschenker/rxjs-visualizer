...
Three pages share one coordinate system. `index.html` is the main demo: a diamond pipeline (source, two `map` branches, a `zip` sink) where every column is a clean top-to-bottom flow, in real time. `glitch.html` runs the same pipeline with a `combineLatest` sink to expose the diamond-dependency glitch (an intermediate sink value); it is postponed work in progress, so keep glitch-specific features there, never on the main page. `spec.html` renders rxjs marble tests (debounceTime, auditTime, mergeMap, switchMap and concatMap so far, picked with `?spec=<name>`) by running them through the real `TestScheduler` in the browser: one frame is one column, expected values are rings, actual values are fills, and a playhead walks virtual time. The design note behind the spec page is `docs/spec-proposal.html` (standalone HTML, also served by the dev server at `/docs/spec-proposal.html`). The plan for the next operators, with session sizing and the per-spec checklist, is `docs/operator-roadmap.md`; update its progress list at the end of a session.

## Commands

```bash
npm run dev       # Vite dev server: http://localhost:5173/ , /glitch.html , /spec.html?spec=<name>
...
Same coordinate rule, different unit: `stepMs: 1` (one TestScheduler frame) and `timeScale: 30` (px per frame), marble 26 px so adjacent frames never overlap. `runMarbleSpec(spec)` constructs `new TestScheduler(capture)` and runs the spec verbatim inside `run()`: `hot()` supplies the source events, the capture callback receives the actual and expected messages from `expectObservable(...).toBe(expected, values)`, and subscription lifetimes are read from the observables' own `subscriptions` logs after the run. State is lanes of `{ frame, kind, value }` events plus spans (subscription, window, cancelled window) and a `playhead`; the only action is `TICK`. Lanes marked `reveal: 'playhead'` show their events when the playhead reaches them; the rest are inputs shown up front.

`MarbleSpec` is a union on `kind`:

- **`'time'`** — one `time()` duration. The spec names its operator-lane annotator via `windows`: `debounceWindows` (a newer value cancels the window) or `auditWindows` (later values are folded in as `inputs`, the window always runs to its end).
- **`'higherOrder'`** — one `cold()` inner re-subscribed per outer value. The spec splits its pipeline into `project(value, inner)` and `flatten(project)` (mergeMap, switchMap, ...) so the runner can wrap `project` with a `tap` stamped with `scheduler.frame`. Each entry in the cold's subscription log becomes a compact **inner lane** (72 px, indented) whose lifetime bar is the log, whose marbles are the tapped emissions of the *projected* inner (values after the spec's `map`), whose character strip is the cold's marble string shifted to its subscription frame, with a drop line from the outer value that spawned it and a small arrow under each emission. Project calls and subscription-log entries line up by index because the flattening operators call `project` at subscription time. When every outer value spawned exactly one inner, the j-th outer value is taken as the spawner of the j-th inner; if its inner was subscribed later than the value arrived (concatMap), the row gets a dotted `queued` span from arrival to subscription and the drop line hangs from the arrival frame. Operators that drop values (exhaustMap) break the index alignment and will need a value-based match plus a "dropped" marker.

Lanes may set `height` (under 90 px switches to the compact layout), `depth`, `noBaseline`, `marblesFrom`, `dropFrom`, `emitArrows`; `laneGeometry()` in the renderer turns that into per-lane Y positions, so lanes are no longer uniform. Adding a spec is one object in `src/specs.ts`; adding an operator family is one annotator plus a `WindowKind`, or nothing at all for another higher-order operator.
...
- **Same-frame order.** Within one frame the scheduler delivers hot-observable messages before operator timers scheduled during the run. So a value on the exact frame a window ends is *seen before the window fires*: auditTime folds it in, debounceTime restarts (cancels) on it. The annotators use `<=` for that reason; keep it.
- **Groups.** `(x|)` advances the ruler by four characters but both events sit on the group's first frame, so a marble and a complete bar can share a frame. `frames` is the longest string's character count, which is why the auditTime spec shows 25 frames for a 22-frame source.
- **Copy specs verbatim from the rxjs repo**, not from memory; the auditTime window is 5 frames and its expected string ends in `(x|)`, which is easy to misremember.
